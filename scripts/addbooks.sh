...
while IFS="|" read -r TITLE AUTHOR SUBNOTE DESCRIPTION IMAGE LULU PRICE; do
	TEXT="<!--- $TITLE ---><div class='entry'><div class='entrycontent'><img class='w-30' src='$IMAGE' alt='$TITLE $AUTHOR' /><div class='flex flex-col gap-2'><div class='flex flex-col gap-1'><h2>$TITLE - $AUTHOR</h2><span class='subnote'>$SUBNOTE</span></div><p>$DESCRIPTION</p></div></div><div class='buttons'><a href='$LULU' target='_blank' class='link-text'>Buy on Lulu - \$$PRICE</a></div></div>"
